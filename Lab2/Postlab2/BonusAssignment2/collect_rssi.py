...
rssi_file_name = "rssi6.csv"  # Output RSSI CSV file name
joystick_file_name = "joystick4.csv"  # Output joystick CSV file name
history = []
...
def moving_average(new_value, window_size = 20):
    global history
...
def rssi2color(rssi):
    ...
    green2red = int(ratio*255)
    if green2red > 255:
        green2red = 255
    if green2red < 0:
        green2red = 0
    return(green2red,255-green2red,0)
...
def captured_packet_callback(pkt):
    ...
    if cur_dict["mac_2"] == dev_mac:
        print(date_time, cur_dict)
        avg_rssi = moving_average(cur_dict["rssi"])
        color = rssi2color(avg_rssi)
        sense.clear(color)
