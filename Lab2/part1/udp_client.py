...
## Display the resulting frame cv2.imshow("Frame", frame) key=cv2.waitKey(1) & 0xFF


    ## frame[rows,columns] --> is the pixel of each frame
    
    ## the above command will only grab the frame
        #message = str(min(3,len(areas))) + "motion detected"
        message = str(top3_area)
