...
while True:
    ...
    for face in faces:
        ...
        smile_position = (int(x+(w/2)), int(y+(h/2)))
        roi = frameGray[y:y+h,x:x+w]
        sr, re, le = smileCascade.detectMultiScale3(roi,2.5,20, outputRejectLevels = True)
        if len(le) == 0:
            cv2.putText(frame, "Not Smiling:(", smile_position, cv2.FONT_HERSHEY_SIMPLEX, 0.75, blue, 3)
        else:
            ...
            # otherwise, there is a smiling in the face ROI
                ...
                cv2.putText(frame, "You are happy:)", smile_position, cv2.FONT_HERSHEY_SIMPLEX, 0.75, blue, 3)
                smile_count +=1
        #sleep(0.1)
    ...
    if(len(faces)>0):
        print("detected faces:",len(faces),"smile faces:",smile_count)
